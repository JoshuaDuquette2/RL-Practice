    ...
    def compute_returns(self, next_value, gamma):
        if self.gae:
            ...
            for step in reversed(range(self.rewards.size(0))):
                ...
                delta_t = self.rewards[step] + (gamma * self.masks[step + 1] * self.value_preds[step + 1]) - self.value_preds[step]
                gae = delta_t + self.gae_lambda * gamma * self.masks[step + 1] * gae
                self.returns[step] = self.value_preds[step] + gae
