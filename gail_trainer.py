...
class GAILTrainer(PPOTrainer):
    ...
    def compute_loss(self, sample):
        ...
        policy_loss_1 = gail_rewards * ratio
        policy_loss_2 = gail_rewards * torch.clamp(ratio, 1 - self.config.ppo_clip_param, 1 + self.config.ppo_clip_param)
    ...
    def update(self, rollout):
        ...
        for epoch in range(1, self.config.discriminator_epoch + 1):  # Run multiple epochs
            ...
            for (expert_data, agent_data) in zip(self.expert_dataloader, data_generator):
                ...
                discriminator_loss = discriminator_loss_func(agent_prediction, torch.zeros_like(agent_prediction)) + discriminator_loss_func(expert_prediction, torch.ones_like(expert_prediction))
